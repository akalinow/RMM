...
//_________________________________________________________________________________________________________
 void SScheme::SetFactors(map <int, Point> & PointMap){
	...
	cout<<endl<<"... Ustawiam czynniki w rownaniu transportu ... "<<endl;

 	map<int,Point>::iterator IT;

	vector<int> flaga; //wektor w ktorym beda trzymane flagi sasiednich punktow dla poszczegolnych pktow na siatce
	flaga.clear();
	...
	//dla wszytskich punktow
	for(IT = PointMap.begin(); IT != PointMap.end(); IT++){
		if (IT->second.get_flag() != 0){ //jesli punkt nie jest poza przeplywem
		  flaga.clear();
		  
		  //wczesniejszy fragment kodu, kiedy funkcja dzialala bez wektora flag sasiadow -- prawdopodobnie do usuniecia
 ...
 void SUpWind::ComputeConcentration (map <int, Point> & PointMap){
	...
	for(IT = PointMap.begin(); IT != PointMap.end(); IT++){
		...
		//if (val > 1e-30) { //nie przepisujemy b malych wartosci
			if (fabs(val) <= 1e-20) val = 0.0; //test 18.09
			//if (val < 0) val = 0.0; //NIE PRZEPISUJEMY UJEMNYCH ....TEST

			if (IT->second.get_flag() !=1  || IT->second.get_flag() == -9999) val = 0.0; //test 18.09
			//if (IT->second.get_flag() != 1) val = 0;
			IT->second.set_Curr(val);
			suma1 = suma1 + IT->second.get_Curr();
			pktpo ++;
		//}
...
//		if (fabs(val) > 1e-40) {
//18.09.2015		if (fabs(val) > 1e-10 && val >= 0) { // zmiana dokladnosci - nie ignoruje berdzo malych wartosci, //usuwa wartosci ujemne  9.3.2010)
//		if (fabs(val) > 1e-10) { // zmiana dokladnosci - nie ignoruje berdzo malych wartosci, // nie usuwa wartosci ujemnych  5.10.2010)
			...
			if (IT->second.get_flag() != 1) val = 0.0;
			IT->second.set_Curr(val);
...
//cout<<endl<<"  suma: "<<suma<<"  suma1: "<<suma1<<"  przed: "<<pktprzed<<"  po: "<<pktpo<<endl<<endl;	
 ...
 void SADI::GetFactors(Point *p, const double &CAx, const double &CAy, const double &CDx, const double &CDxy, const double &CDy, const vector<int> &flag){
	...
	//dla zadanego punku ustawiamy poszcegolne wartosci wspolczynnikow - ponizej wartosci odpowiednie gdy punkt znajduje sie w przeplywie
